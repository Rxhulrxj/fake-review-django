...
def productslist(request):
    ...
    return render(request,'productslist.html',{'products':products,'user':user})
...
def writereviews(request,product_id):
    if request.method == "POST":
        ...
        if detail:
            ...
            status = 'view'
            messages.error(request,"Purchase product to write a review....")
    productreviews = Review.objects.filter(product_id=product_id)
    return render(request, 'writereview.html', {'productreviews': productreviews,'status':status})
def viewreviews(request,product_id,status):
    status = status
    productreviews = Review.objects.filter(product_id=product_id)
